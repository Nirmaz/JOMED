...
def init_distributed_mode(params):
    ...
    warnings.warn("START WORK ON DIS")
    ...
    if params.is_slurm_job and has_local_rank:
        ...
        warnings.warn(f"Firstv if define")
        warnings.warn(f"RANK {str(params.global_rank)}")
        warnings.warn(f"world size {str(params.world_size)}")
        warnings.warn(f"MASTER_ADDR {str(params.main_addr)}")
        warnings.warn(f"MASTER_PORT {str(params.main_port)}")
    ...
    # multi-GPU job (local or multi-node) - jobs started with torch.distributed.launch
        ...
        warnings.warn(f"second if define")
        warnings.warn(f"RANK {str(params.global_rank)}")
        warnings.warn(f"world size {str(params.world_size)}")
        warnings.warn(f"n gpu per node {str(params.n_gpu_per_node)}")
